...
class leap():
    ...
    def __init__(self,master):
        ...
        file1_label = Label(root,bg = 'white',text='Select Data/Keywords Column',font='Calibri 10 bold').grid(row=11, sticky=W,padx=4,pady=(0, 15))
        #self.column_list= []
        self.column_chosen= OptionMenu(root,self.n,'Select')
        #self.column_chosen.config(bg='white')
        self.n.set('Select')
        self.column_chosen.configure(background="white", activebackground="white",bg='white')
        self.column_chosen['menu'].configure(bg='white')
        self.column_chosen['highlightthickness']=0
        self.column_chosen.grid(column = 1, row = 11, sticky = E, padx=4,pady=(0,15))
    ...
    def retrain(self):
        try:
            if self.var.get() ==1:
                ...
                self.df_mapping = pd.read_excel(self.filename2.name)
                ...
                shutil.copy('count_vect_BS.pkl', r'count_vect_BS_%s.pkl' % x)
                os.remove('count_vect_BS.pkl')
                pickle.dump(count_vect, open('count_vect_BS.pkl', 'wb'))
                ...
                shutil.copy('count_vect_PL.pkl', r'count_vect_PL_%s.pkl' % x1)
                os.remove('count_vect_PL.pkl')
                pickle.dump(count_vect, open('count_vect_PL.pkl', 'wb'))
    ...
    def open_file1(self):
        try:
            ...
            for columns in self.df1.columns.values:
                ...
                #self.column_list.append(columns)
                p += 1
            self.lb.update_idletasks()

            for values in self.df1.columns.values:
                self.column_list.append(values)

            self.column_chosen= OptionMenu(root,self.n,*self.column_list)
            self.n.set(self.column_list[0])
            self.column_chosen.configure(background="white", activebackground="white")
            self.column_chosen['menu'].configure(bg='white')
            self.column_chosen['highlightthickness'] = 0
            self.column_chosen.grid(column=1, row=11, sticky=E, padx=4, pady=(0, 15))
            ...
            messagebox.showerror('Import Error','Some error ocurred while importing data file.')
            ...
            if self.v1.get()==1:
                ...
                if self.var.get()==1:
                    ...
                    with open('PL.pkl', 'rb') as f:
                        SVM_LEAP_clf = pickle.load(f)
                        f.close()
                    count_vect = pickle.load(open('count_vect_PL.pkl', 'rb'))
                    # SVM_LEAP_clf = pickle.load(self.resource_path('SVM_LEAP.pkl'))
                    # count_vect = pickle.load(self.resource_path('count_vect_LEAP'))
                    self.result = []
                    select_columns_entry1=self.n.get()
   ...
   # def get_consolidated_result(self):
    ...
    def final_result_Fuzzy(self):
        ...
        try:
            ...
            select_columns_entry1 = self.n.get()
            select_columns_entry2 = self.n.get()
            #select_columns_entry1 = self.file1_column.get()
