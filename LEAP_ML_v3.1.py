...
class leap():
    ...
    def __init__(self,master):
        ...
        self.label_fuzz.grid(row=6,sticky=W,padx=4,pady=(0, 15))
        ...
        self.button_fuzz.grid(row=6,column=1, sticky=E,padx=4,pady=(0,10))
        ...
        self.frm_model.place(x=0,y=6,width=100, height=100)
        self.var.set(1)
    ...
    def multi_thread1(self):
        ...
        k = iter(list(self.df1[select_columns_entry1]))
        ...
        #time_Consumed = (time.process_time() - startTime) / 60
        #messagebox.showinfo("Process Completed", "The process has been completed in " + str("%0.2f" % time_Consumed) + " minutes !!!")
 #   def consolidated_result_fratio(self):
        self.selected_cols_list=[]
        df_list_keywords_level4=[]
        self.selected_cols = self.lb.curselection()
        #print(self.selected_cols)
        ...
        self.result_match1 = []
        for k in self.merged_df1['Score']:
            if (int(k) >= 70):
    ...
    def multi_thread(self):
        ...
        #batch_size = 15
        #while (length - (batch_size * count)) > 0:
        i = iter(list(self.df1[select_columns_entry1]))
        for j in range(len(self.df1[select_columns_entry1])):
   ...
   # def get_consolidated_result(self):
    ...
    def final_result_Fuzzy(self):
        try:
            ...
            #print(self.final_df1,'Final df partial ratio')
            #print(self.final_df,'Final df token sort ratio')
            self.df3 = pd.DataFrame()
            self.df3.empty

            for b in self.final_df1.index.values:
                if int(self.final_df1['Score'][b]) >= int(self.final_df['Score'][b]):
                    self.df_temp = self.final_df1.iloc[[b]]
                    self.df3 = self.df3.append(self.df_temp.iloc[0], ignore_index=True)
                    self.df3=self.df3.reindex(self.df_temp.columns, axis=1)
                    del self.df_temp
                elif int(self.final_df['Score'][b])>int(self.final_df1['Score'][b]):
                    self.df_temp = self.final_df.iloc[[b]]
                    self.df3 = self.df3.append(self.df_temp.iloc[0], ignore_index=True)
                    self.df3 = self.df3.reindex(self.df_temp.columns, axis=1)
                    del self.df_temp
                else:
            ...
            print(self.df3)
            export_final_excel = self.df3.to_excel(os.getcwd()+'\Final_Output_Fuzzy.xlsx', index=None, sheet_name='Final_Output')
            messagebox.showinfo('Completed','Completed,please check Final_Output and Variance sheets')
        except:
